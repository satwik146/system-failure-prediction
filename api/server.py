...
async def _push_loop():
    while True:
        ...
        if not ws_manager._clients: continue
        try:
            await ws_manager.broadcast({
                "type": "tick", "timestamp": time.time(),
                "bus_stats": _bus.stats if _bus else {},
                "agent": _agent.latest_results() if _agent else {},
        ...
        except Exception as e: log.debug("WS push: %s", e)

@asynccontextmanager
async def _lifespan(app):
    asyncio.create_task(_push_loop(), name="ws-push"); yield
...
async def root():
    f = DASH / "index.html"
    return FileResponse(str(f)) if f.exists() else {"status": "ok"}
...
async def status():
    try:
        ...
        agent_results = _agent.latest_results() if _agent else {}
        agent_health = _agent.system_health() if _agent else 100.0
        sim_status = _simulator.status() if _simulator and hasattr(_simulator, 'status') else {}
        return {
            "bus": bus_stats,
            "system_health": agent_health,
...
async def predictions(): return _agent.latest_results() if _agent else {}
...
async def run_scenario(name: str):
    if not _simulator: return {"error": "no simulator"}
...
async def inject(body: dict):
    ...
    h = _simulator.injector.inject(
        body.get("source_id","unknown"), body.get("tag","value"),
...
async def cancel():
    if _simulator: _simulator.injector.cancel_all()
